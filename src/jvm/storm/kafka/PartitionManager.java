...
public class PartitionManager {
    ...
    ConsumerConnector high_consumer;
    ZkState _state;
    Map _stormConf;
    ...
    public PartitionManager(DynamicPartitionConnections connections, String topologyInstanceId, ZkState state, Map stormConf, SpoutConfig spoutConfig, Partition id) {
        ...
        try {
        	  long offset=_state.readData(path);
...
//            _state.writeJSON(committedPath(), data);
            ...
            _state.writeData(committedPath(), String.valueOf(lastCompletedOffset));

            _committedTo = lastCompletedOffset;
            LOG.debug("Wrote last completed offset (" + lastCompletedOffset + ") to ZK for " + _partition + " for topology: " + _topologyInstanceId);
        } else {
            LOG.debug("No new offset for " + _partition + " for topology: " + _topologyInstanceId+",_committedTo"+_committedTo
            		+",lastCompletedOffset"+lastCompletedOffset);
        }
    }

    private String committedPath() {
        return _spoutConfig.zkRoot + "/" + _spoutConfig.id + "/offsets/"+_spoutConfig.topic+"/" + _partition.getId();
    }

    public long lastCompletedOffset() {
        if (_pending.isEmpty()) {
 //       	LOG.warn("_pending is empty");
